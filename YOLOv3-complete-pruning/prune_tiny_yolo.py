...
class opt():
    model_def = "D:\HZQ\jtrafficsigns\YOLOv3-complete-pruning\cfg\yolov3-tiny-3l-9a-cbam-hzq.cfg"
    data_config = "D:\HZQ\jtrafficsigns\YOLOv3-complete-pruning\data\CCTSDB.data"
    model = 'D:\HZQ\jtrafficsigns\YOLOv3-complete-pruning/results\prune2\prune2-weights/best.pt'
    # model_def = "/home/hzq/study/yolov3-prune/YOLOv3-complete-pruning-copy/cfg/yolov3-tiny.cfg"
    # data_config = "/home/hzq/study/yolov3-prune/YOLOv3-complete-pruning-copy/data/CCTSDB.data"
...
def obtain_filters_mask(model, thre, CBL_idx, prune_idx):
    ...
    for idx in CBL_idx:  # [0, 2, 4, 6, 8, 10, 12, 13, 14, 19, 22, 27, 30]
        bn_module = model.module_list[idx][
            1]  # BatchNorm2d(16, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        if idx in prune_idx:  # [0, 2, 4, 6, 8, 10, 12, 13, 14, 19, 22, 27, 30]

            mask = obtain_bn_mask(bn_module, thre).cpu().numpy()
            remain = int(mask.sum())
            pruned = pruned + mask.shape[0] - remain  # 0+16-11
...
# CBL_idx=[0, 2, 4, 6, 8, 10, 12, 13, 14, 19, 22, 27, 30]
...
def init_weights_from_loose_model2(compact_model, loose_model, CBL_idx, Conv_idx, CBLidx2mask):
    ...
    for idx in Conv_idx:
        ...
        in_channel_idx = np.argwhere(input_mask)[:, 0].tolist()  # np.argwhere：返回非0的数组元组的索引， tolist将数组或者矩阵转换成列表
        compact_conv.weight.data = loose_conv.weight.data[:, in_channel_idx, :, :].clone()
        # compact_conv.weight.data = loose_conv.weight.data[:,512,:,:].clone()
